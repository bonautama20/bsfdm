...
const router = Router();
// Employee management is paid-only — see the multi-tenant plan's Phase 3.
router.use(requirePlan("Employee"));
...
router.get("/", (req, res) => {
  res.json(req.db.prepare("SELECT * FROM employees ORDER BY rowid").all().map(toEmployee));
});
...
router.post(
  ...
  const id = nextId(req.db, "employees", "EMP", 3);
  req.db.prepare(
  ...
  res.status(201).json(toEmployee(req.db.prepare("SELECT * FROM employees WHERE id = ?").get(id)));
});

router.patch("/:id", requirePermission("Employee", "edit"), validateBody(employeeSchema), (req, res) => {
  const existing = req.db.prepare("SELECT * FROM employees WHERE id = ?").get(req.params.id);
  if (!existing) return res.status(404).json({ error: "Employee not found." });

  const b = req.body || {};
  req.db.prepare("UPDATE employees SET name=?, position=?, department=?, phone=?, email=?, status=? WHERE id=?").run(
  ...
  res.json(toEmployee(req.db.prepare("SELECT * FROM employees WHERE id = ?").get(req.params.id)));
});

router.delete("/:id", requirePermission("Employee", "delete"), (req, res) => {
  const result = req.db.prepare("DELETE FROM employees WHERE id = ?").run(req.params.id);
...
router.get("/attendance/today", (req, res) => {
  ...
  res.json(req.db.prepare("SELECT * FROM attendance WHERE date = ? ORDER BY rowid").all(today).map(toAttendance));
});

router.get("/:id/attendance", (req, res) => {
  res.json(req.db.prepare("SELECT * FROM attendance WHERE employee_id = ? ORDER BY date DESC").all(req.params.id).map(toAttendance));
});
